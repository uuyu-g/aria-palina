...
プロジェクトの要件に合わせて、必要なパッケージをインストールしてください。

```bash
# グローバルに CLI / TUI ツールをインストールする場合
$ npm install -g @aria-palina/cli @aria-palina/tui

# プロジェクトのテスト依存としてインストールする場合
$ npm install -D @aria-palina/test-utils
```

## 🖥️ 1. CLI モード (`aria-palina-cli`)

Unix パイプラインと組み合わせて、アクセシビリティツリーを抽出・フィルタリングするためのワンショットコマンドです。出力先がターミナルかパイプかを自動検知し、最適なフォーマットを出力します。

### 基本コマンド

```bash
$ aria-palina-cli --url <ターゲットURL> [オプション]
```
...
| `--headed` | `false` | ヘッドレスモードを無効にし、ブラウザの GUI を表示する |

### 💡 パイプ処理のハック（Unix Philosophy）

**例1: `aria-expanded` が true になっている（展開されている）コンボボックスを探す**

```bash
$ aria-palina-cli -u http://localhost:3000 -f json | jq '.[] | select(.role == "combobox" and .state.expanded == true)'
```

**例2: 画像（`role="img"`）のうち、`alt` (Name) が空になっているものを探す**

```bash
$ aria-palina-cli -u http://localhost:3000 -f json | jq '.[] | select(.role == "img" and .name == "")'
```
...
$ aria-palina-cli -u http://localhost:3000 -f text | grep "^\[ボタン\]" | wc -l
```

## 🕹️ 2. TUI モード (`aria-palina-tui`)

ターミナル上で NVDA の挙動を模倣し、インタラクティブに DOM ツリーをナビゲートする没入型デバッグツールです。

### 基本コマンド

```bash
$ aria-palina-tui --url <ターゲットURL> [オプション]
```

### 🌟 Two-way Matrix View (Headed モード)

`--headed` オプションを付けて起動すると、ブラウザが立ち上がります。TUI でカーソルを動かすと、ブラウザ上の対応する要素が **青くハイライト** され、視覚とセマンティクスのズレを一瞬で特定できます。

```bash
$ aria-palina-tui --url http://localhost:3000 --headed
```
...
| `H` | クイック | 次の「見出し（Heading）」へジャンプ |
| `Shift + H` | クイック | 前の「見出し」へジャンプ |
| `D` | クイック | 次の「ランドマーク（`main`, `nav` 等）」へジャンプ |
| `Enter` | アクション | 現在フォーカスしている要素のクリックを発火（リンク遷移やボタン展開） |
| `Q` または `Ctrl+C` | システム | TUI を終了し、バックグラウンドのブラウザを閉じる |

## 🧩 3. Chrome DevTools 拡張機能
...
## 🧪 4. 自動テストユーティリティ (`@aria-palina/test-utils`)
...
import { ariaPalinaMatchers } from '@aria-palina/test-utils';

expect.extend(ariaPalinaMatchers);
```

### API リファレンス

#### `toHaveA11yTextSequence(sequence: string[])`
...
test('モーダル内の正しいフォーカス順序', async ({ page }) => {
  ...
  await expect(page).toHaveA11yTextSequence([
    '[ダイアログ] 設定',
    '[テキスト入力] ユーザー名',
    '[ボタン] 保存',
    '[ボタン] キャンセル'
...
#### `not.toHaveA11yText(text: string)`
...
test('モーダル展開時、背景要素が隠蔽されること', async ({ page }) => {
  ...
  await expect(page).not.toHaveA11yText('[リンク] 利用規約');
});
```
...
A. `npm run cli` のように npm script 経由で実行すると、npm 自身の出力ログが `stdout` に混ざる場合があります。`npx aria-palina-cli` または `pnpm aria-palina-cli` のように直接バイナリを叩くか、`--silent` フラグを使用して npm のログを抑制してください。
