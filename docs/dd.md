...
export interface A11yNode {
  backendNodeId: number;     // DOMのハイライト同期に使用
...
### 3.3 Matrix View (Headed モード同期)

TUI モードで `--headed` が指定された場合、または DevTools 拡張機能の場合、CDP の `Overlay` ドメインを使用してブラウザ上の要素をハイライトします。TUI カーソル → ブラウザ画面の片方向同期のみを提供し、逆方向 (ブラウザ上の操作を TUI に反映) はロードマップ外とする。
