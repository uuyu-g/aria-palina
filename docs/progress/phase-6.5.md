...
- **ランドマーク入れ子はスタックで保持**: `<banner><nav>...</nav></banner>`
  ...
  追加し続ける。
- **描画は左レール半ボックス (閉じない派)**: セクション見出しは `┌── label`
  (その rail 位置で初めて開くセクション) または `├── label` (同 or 浅い rail に
  既存セクションがあるときの続き) で描く。閉じ (`└──`) は意図的に出さない。
  ライブ更新・仮想スクロールで再描画する際に「閉じ線の位置」を追跡する必要が
  なくなり、セクションの途中が可視外に切れても矛盾が生じないため。
  アイテム行は祖先ランドマーク数ぶん `│ ` レールを左端に垂らす。
- **`<Box borderStyle>` は採用しない**: Ink の `<Box>` borderStyle で章を囲う
  案も検討したが、セクション全体を Yoga に渡す必要があり、現行の行ベース仮想
  スクロール (可視分だけ slice する `computeWindow`) と本質的に相性が悪い。
  大規模ページで `main` 内に数千ノードがあると毎フレームの flexbox 計算が重く
  なるため、自前 ASCII の左レールで描画する。
