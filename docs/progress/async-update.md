...
## 追記: SPA ロード進行を拾う細粒度 mutation 購読 (#40)

`#40` で判明した欠落を解消するため、`subscribeAXTreeUpdates` の購読対象に
DOM mutation 系イベントを追加した。

### 背景

既存の購読は粗粒度 3 種 (`DOM.documentUpdated` / `Page.frameNavigated` /
`Page.lifecycleEvent`) に限定していたため、以下のような段階的レンダリングが
TUI のライブ更新に反映されない問題があった:

- ローダ UI → コンテンツ置換 (childNode 差し替えだけで documentUpdated が
  発火しない SPA)
- `aria-busy="true"` → `"false"` の属性トグル
- スケルトン → 実データ差し替え
...
| イベント | 粒度 | 反映される典型シナリオ |
| -------- | ---- | ---------------------- |
| `DOM.childNodeInserted` | 細 | 非同期で現れる検索結果・フィード |
| `DOM.childNodeRemoved`  | 細 | スケルトン/ローダの撤去 |
| `DOM.attributeModified` | 細 | `aria-busy` / `aria-expanded` 等のトグル |

`aria-busy` の明示的な true → false 追従は、`attributeModified` 経由で
自然に拾える (確定操作後の明示 refresh と同じ粒度)。

### デバウンス設計 — 粗粒度と細粒度の 2 段分離

細粒度は発火数が桁違いに多いため、**粗粒度と別のデバウンス値**を持たせる。

| 種別 | 既定デバウンス | `AXUpdateOptions` キー |
| ---- | --------------- | ---------------------- |
| 粗粒度 (documentUpdated / frameNavigated / lifecycleEvent) | 200ms | `debounceMs` |
| 細粒度 (childNodeInserted/Removed / attributeModified) | 400ms | `mutationDebounceMs` |

同一デバウンスウィンドウに複数イベントが重なった場合は、
cause 優先順位 `manual > navigation > document > lifecycle > mutation` で
最も「確からしい」原因を `onUpdate` に渡す。細粒度バースト中に粗粒度イベント
が割り込んだときは、粗粒度のデバウンス (短い方) で timer が再設定されるため、
ロード完了の節目では追従速度が落ちない。

### トレードオフ (負荷 vs 追従性)

- `Accessibility.getFullAXTree` の呼び出しコストは無視できないが、400ms の
  静穏化 + 単一実行ロック (進行中の抽出完了まで次を待たせる) で連続変更
  1 回ぶんに収束する。
- 更新頻度が極端に高いページ (リアルタイムダッシュボード等) で負荷が問題に
  なる場合は、`AXUpdateOptions.subscribeMutations: false` で粗粒度のみへ
  縮退できる。`@aria-palina/core` に閉じた設定で、TUI からは `LiveBridge`
  越しに将来のフラグを追加する余地を残した。
- `DOM.childNodeInserted` は overlay / ハイライトなど内部描画でも発火する
  可能性があるが、同じ 400ms デバウンスで吸収されるため観測上のノイズに
  ならない。

### テスト

`packages/core/src/__tests__/subscribe-ax-updates.test.ts` に 5 ケース追加
(全 12 ケース):

- `DOM.childNodeInserted` が `mutationDebounceMs` 経過後に mutation cause
  で発火する (粗粒度 debounce では発火しないことも確認)
- `DOM.childNodeRemoved` / `DOM.attributeModified` も同様に mutation cause
  で発火する
- 連続する mutation バーストは 1 回にまとめられる
- mutation 中に粗粒度イベントが割り込むと cause は粗粒度側が優先される
- `subscribeMutations: false` で mutation 系購読がスキップされ、粗粒度は
  引き続き動作する

加えて既存の unsubscribe テストで、mutation 系リスナーも off されている
ことを検証する。

### 公開 API 変更

- `AXUpdateCause` に `"mutation"` を追加
- `AXUpdateOptions` に次を追加 (後方互換。未指定なら既定値で自動有効化):
  - `mutationDebounceMs?: number` (既定 400)
  - `subscribeMutations?: boolean` (既定 true)
