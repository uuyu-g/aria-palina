...
> `aria-palina` monorepo の**プロジェクト規約**を記した運用メモです。
> コードを書き始める前に必ず目を通し、矛盾する判断を避けてください。

## 🗣️ 応答言語スタイル

**プロジェクト規約**: Claude (および他の AI エージェント) がユーザーへ返す
**自然言語の応答は、常に日本語のですます調で書くこと**。

- 対象: チャット応答、ツール呼び出し前後の一文アナウンス、最終サマリ、
  質問・確認メッセージなど、ユーザーに表示されるすべてのテキスト。
- 口調: 素直な「です・ます」調を基本とし、断定形 (「〜する」「〜した」) や
  命令形 (「〜しろ」「〜せよ」) を避ける。
- 敬語 (尊敬語・謙譲語) は使わない。
  「いたします」「させていただきます」「ご確認ください」のような
  過度にかしこまった表現はよそよそしいため避け、
  「します」「しますね」「確認してください」のようにフラットに書く。
- 例外:
  - ソースコード、コミットメッセージ、PR タイトル・本文、ファイル内コメントは
    従来どおりの記法 (英語・日本語常体混在可) で書いてよい。
  - 本ファイル自身や `docs/*.md` 等の**既存ドキュメント本文の文体**は、
    このルールの対象外 (ドキュメントは引き続き常体で書かれる)。

#### ✅ 良い例

> ファイルを確認します。
> `CLAUDE.md` を更新しました。次に `vp check` を実行します。

#### ❌ 悪い例 (常体 — ですます調になっていない)

> ファイルを確認する。
> CLAUDE.md を更新した。次に vp check を実行する。

#### ❌ 悪い例 (敬語 — よそよそしい)

> ファイルを確認いたします。
> `CLAUDE.md` を更新させていただきました。次に `vp check` を実行いたします。
...
| `src/tui/components/App.tsx`         | ヘッダー / VirtualList / フッターの 3 段。`useInput` でキーバインド、`useMouse` でマウスホイール処理 |
...
**キーバインド**: `↑`/`↓`/`j`/`k`/マウスホイール (行移動), `PageUp`/`PageDown` (ページ移動), `g`/`G` (先頭/末尾), `Tab`/`Shift+Tab` (インタラクティブ), `h`/`H` (見出し), `D` (ランドマーク), `r`/`R` (AX ツリー再取得), `L` (ライブ更新トグル), `q`/`Ctrl+C` (終了)。
